...
class EventsController extends AbstractController
{
    ...
    {
        $eventType = $request->query->get('type');                              // get type from URL

        if ($eventType) {                                                       //check conditions. if event type excists - 
            $events = $eventsRepository->findBy(['type' => $eventType]);        //controller checks matching types, using findBy (type)
        } else {
            $events = $eventsRepository->findAll();                             //if no events chosen, show all events
        }

        return $this->render('events/index.html.twig', [                        //shows all filtered events
            'events' => $events,
        ]);
    }
